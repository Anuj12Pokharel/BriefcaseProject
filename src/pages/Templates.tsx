...
export default function Templates() {
  ...
  const handleTemplateTypeChange = (type: 'document' | 'message') => {
    ...
    // reflect selection in the URL so top-nav and deep links can open the right view
    try {
      navigate(`/templates?type=${type}`);
    } catch (e) {}
  };

  // sync from URL query param `type` -> templateType
  const location = useLocation();
  useEffect(() => {
    try {
      const qp = new URLSearchParams(location.search);
      const t = qp.get('type');
      if (t === 'message' || t === 'document') {
        setTemplateType(t as 'document' | 'message');
      }
    } catch (e) {}
  }, [location.search]);
